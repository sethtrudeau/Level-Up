...
function PlantSprite({ color, stage }: { color: string; stage: number }) {
  const pot = (
    <>
...
function GardenPlant({ feature, left, top, stage, done, onClick }: {
  feature: Feature; left: number; top: number; stage: number; done: boolean; onClick: () => void;
}) {
...
export default function GardenView({ features, level, onPick }: {
