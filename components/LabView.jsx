// Lab theme: high-school chemistry classroom. Benches arranged in neat rows &
// columns with aisles between, each holding one piece of equipment.
window.LabView = function LabView({ features, level, onPick, onBackToHub }) {
  const progress = window.useProgress();

  // Classroom layout: 2 rows of 3 student benches + 1 demo bench at the front.
  // Grid columns at x = 200, 500, 800; rows at y = 220, 460, 680.
  // Index 0..6 → Adaptive Memory, Image, Voice, Doc Editor, Python, Conversation Review, References
  const stations = [
    { x: 200, y: 220, equip: 'cortex'      }, // row 1, col 1
    { x: 500, y: 220, equip: 'scope'       }, // row 1, col 2
    { x: 800, y: 220, equip: 'oscilloscope'}, // row 1, col 3
    { x: 200, y: 460, equip: 'plotter'     }, // row 2, col 1
    { x: 500, y: 460, equip: 'centrifuge'  }, // row 2, col 2
    { x: 800, y: 460, equip: 'terminal'    }, // row 2, col 3
    { x: 500, y: 680, equip: 'archive'     }  // demo bench, front center
  ];
  ...
  return (
    <div style={{
      ...
      {/* Floor: grid paper + aisles + door only (no benches here) */}
      <svg
      ...
      >
        ...
        {/* Classroom aisles: subtle dashed wayfinding between rows and columns */}
...
function LabStation({ feature, left, top, equip, done, onClick }) {
  ...
  return (
    ...
    >
      {/* Dark lab-bench edge strip along the top */}
      <div style={{
        height: 10, background: '#4a5068',
        borderBottom: '1px solid #2a3248', flexShrink: 0
      }}/>

      {/* Atomic chip + status — centered below the edge strip */}
      <div style={{
      ...
      {/* Equipment — centered, takes the middle */}
      <div style={{
        flex: 1,
      ...
      {/* Name plate — centered at the bottom */}
      <div style={{
...
// Top-down/front lab equipment, ~130w x 96h footprint, base at y=96
function LabEquipment({ kind, color }) {
  const W = 130, H = 96;
  const common = { width: W, height: H, style: { display:'block' } };

  if (kind === 'cortex') {
    // brain-in-a-jar — adaptive memory
    return (
  ...
  if (kind === 'oscilloscope') {
    // scope with waveform — voice input
    return (
      <svg viewBox={`0 0 ${W} ${H}`} {...common}>
  ...
  if (kind === 'plotter') {
    // pen plotter drawing on paper — document editor
    return (
      <svg viewBox={`0 0 ${W} ${H}`} {...common}>
        <rect x="10" y="84" width="110" height="8" fill="#9aa3ba" rx="2"/>
        {/* paper */}
  ...
  if (kind === 'terminal') {
    // computer terminal — conversation review
    return (
      <svg viewBox={`0 0 ${W} ${H}`} {...common}>
        <rect x="10" y="84" width="110" height="8" fill="#9aa3ba" rx="2"/>
        {/* keyboard */}
  ...
  if (kind === 'archive') {
    // filing cabinet + books — references
    return (
      <svg viewBox={`0 0 ${W} ${H}`} {...common}>
        <rect x="10" y="84" width="110" height="8" fill="#9aa3ba" rx="2"/>
        {/* cabinet */}
