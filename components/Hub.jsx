// Landing hub: 3 portal cards. Order of first entry determines which level lives in which place.
window.Hub = function Hub({ features, onLogin }) {
  ...
  const allVisited = state.visitOrder.length >= 3;
  const allFinished =
...
function Portal({ portal, featureCount, assignedLevel, pendingLevel, completedCount, finished, onEnter }) {
  ...
  return (
    ...
    >
      ...
      {/* Cleared badge (only shown when theme is finished) */}
      {finished && (
        <div style={{
  ...
  if (kind === 'garden') {
    return (
      <svg viewBox="0 0 400 340" preserveAspectRatio="xMidYMid slice" style={{width:'100%', height:'100%'}}>
        ...
        {/* hills raised so they frame the middle */}
        ...
        {/* plants planted along the middle band */}
