...
export default function ChallengeModal({ feature, theme, level, onClose }: {
  feature: Feature;
  theme: 'garden' | 'lab' | 'playground';
  level: number;
  onClose: () => void;
}) {
  ...
  const themeSpec = feature[theme] as unknown as Record<string, string>;
  const themeChrome = themeSpec?.plant || themeSpec?.symbol || themeSpec?.structure || '';
