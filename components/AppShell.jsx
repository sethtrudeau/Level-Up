...
window.AppShell = function AppShell() {
  ...
  const [active, setActive] = React.useState(null); // selected feature for modal
  ...
  if (state.screen === 'hub' || !state.currentTheme) {
    return (
      <>
        <window.Hub features={features} onLogin={() => setLoginOpen(true)} />
        {loginOpen && <window.LoginModal onClose={() => setLoginOpen(false)} />}
      </>
    );
  }
  ...
  const View =
    theme === 'garden' ? window.GardenView :
    theme === 'lab'    ? window.LabView :
                         window.PlaygroundView;
