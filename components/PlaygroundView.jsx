// Playground theme: a spatial scene — features laid out around a park
// with structures placed freely across the canvas, not in a grid.
window.PlaygroundView = function PlaygroundView({ features, level, onPick, onBackToHub }) {
  const progress = window.useProgress();

  // Freely placed stops on a 1000x700 scene.
  // Each index maps to features[0..6]: Adaptive Memory, Image, Voice,
  // Doc Editor, Python, Conversation Review, References.
  const stops = [
    { x: 160, y: 200 }, // NW — swing
    { x: 490, y: 150 }, // N   — slide
    { x: 830, y: 220 }, // NE  — megaphone
    { x: 220, y: 500 }, // SW — sandbox
    { x: 520, y: 430 }, // center — climber
    { x: 820, y: 500 }, // SE — seesaw
    { x: 360, y: 640 }  // bottom — treehouse
  ];
  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 1000 800" preserveAspectRatio="none" style={{position:'absolute', inset:0, width:'100%', height:'100%', pointerEvents:'none'}}>
        ...
        {/* rolling hills behind — lowered so they don't hit the header */}
        ...
        {/* a couple of trees in background */}
...
function PlayStop({ feature, left, top, done, onPick }) {
  ...
  return (
    ...
    >
      {/* the structure itself, standing on the grass */}
      <div style={{
        position: 'relative',
...
function StructureSVG({ kind, color }) {
  // Each structure ~150x130, positioned so its base sits at bottom of the viewBox.
  const W = 150, H = 130;
  ...
  if (kind === 'slide') {
    return (
      <svg viewBox={`0 0 ${W} ${H}`} {...common}>
        ...
        {/* slide ramp — two parallel rails with fill between */}
